...
function TScharts() {
  ...
  function getTSSeries() {
    ...
    axios
      ...
      .then((res) => {//console.log(res.data.states_daily[0].tg)
        stlength=res.data.states_daily.length
        for(let i=0;i<stlength;i++){
          if(res.data.states_daily[i].status==="Confirmed" & res.data.states_daily[i].tg!=null){
            stconfirmed[res.data.states_daily[i].date]=res.data.states_daily[i].tg
          }
          else if(res.data.states_daily[i].status==="Recovered" & res.data.states_daily[i].tg!=null){
            strecovered[res.data.states_daily[i].date]=res.data.states_daily[i].tg
          }
          else if(res.data.states_daily[i].status==="Deceased" & res.data.states_daily[i].tg!=null){
            stdeceased[res.data.states_daily[i].date]=res.data.states_daily[i].tg
          }
  ...
  function getTSdistrictstopfive() {
    ...
    axios
      ...
      .then((res) => {res.data.map((cases)=>{
        if(cases.state==="Telangana"){
        
          cases.districtData.map((dat)=>{
            if(dat.district!=='Unknown'){
              listy1.push({district:dat.district,value:dat.confirmed})
              listy2.push({district:dat.district,value:dat.active})
              listy3.push({district:dat.district,value:dat.recovered})
              listy4.push({district:dat.district,value:dat.deceased})
  ...
  return (
    <div className="container-fluid">
      <h3>Covid-19 Telangana State Dashboard</h3>
  ...
  <div className="col-lg-2"  style={{backgroundColor: "#70a0be",color:"white",fontSize:"25px"}}><h3 style={{color:"white"}}>Active</h3><br />{active}</div><br/>
  <div className="col-lg-1"></div>
      <div className="col-lg-2" style={{ backgroundColor: "#61c1a1",color:"white",fontSize:"25px"}}><h3 style={{color:"white"}}>Recovered </h3><br />{recovered}</div><br/>
      <div className="col-lg-1"></div>
  <div className="col-lg-2" style={{backgroundColor: "#5f575f",color:"white",fontSize:"25px"}}><h3 style={{color:"white"}}>Deaths</h3><br />{deaths}</div><br/>
    ...
    <div className="container" >
          ...
          <div className="row">
            ...
            <BarChart data={topfivedeaths} ></BarChart>
            </div>
